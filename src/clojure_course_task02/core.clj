...
(defn get-files-list [fil dir]
  (let[files (.listFiles dir)
       dirs (filter #(.isDirectory %) files)]
  (def  fl(future (map #(.getName %) (filter #(and (.isFile %) (isMtch fil (.getName %))) files))))
  (flatten (concat @fl (pmap #(get-files-list fil %) dirs)))))


(defn find-files [file-name path]
  "TODO: Implement searching for a file using his name as a regexp."
  (get-files-list file-name (File. path)))
